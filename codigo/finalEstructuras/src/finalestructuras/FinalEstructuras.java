package finalestructuras;

import arbolB.estructura;
import graficos.principal;

public class FinalEstructuras {

    public static void main(String[] args) {
        graficos.principal nuevoGrafico = new principal();
        nuevoGrafico.setVisible(true);
        int datos [] = {20,10,12,15};
        String lugares [] = {"guate","SM","HU","R"};
        arbolB.estructura arbol [] = new estructura[5];
        for (int i = 0; i < datos.length; i++) {
            arbol = arbolB.insertar.insetarDato(datos[i],arbol,false,lugares);
        }
        System.out.println("inserto");
//        arbolB.estructura nuevaEstructura[] = new estructura[5];
//        //int datosInsertar [] = {35,30,29,25,18};
//        //int datosInsertar [] = {18,25,29,30,35};        
//        int datosInsertar [] =   {35,3,12,2,45,6,4,1,7,8,9,36,44,60,30,31,41,32,33,34,61,62,63};
//        for (int i = 0; i < datosInsertar.length; i++) {
//            if(datosInsertar[i]==63)
//                System.out.println("le toca expandir con tope en el anterior");
//            nuevaEstructura = arbolB.insertar.insetarDato(datosInsertar[i], nuevaEstructura,false);
//            System.out.println("inserto "+datosInsertar[i]);
//            System.out.println("los datos totales son los siguientes");
//        for (int j = 0; j < nuevaEstructura.length; j++) {
//            if(nuevaEstructura[j] != null){
//                System.out.print(nuevaEstructura[j].dato);
//                System.out.print("|")   ;
//            }
//        }
//            System.out.println("");            
//        }
//        
//        System.out.println("fin");
    }
    
}
